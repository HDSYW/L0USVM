function [ PredictY , model ] = LIB_L2SVC( ValX , Trn , Para )
% min  0.5*norm(w,2)^2 + C*xi'*xi , 
...
%% Input 
   ...
   elseif ktype=="poly" % ________ (gamma*u'*v + coef0)^degree
        ...
        coef0 = 1; 
        degree = kp1; 
        lib_opt =  sprintf('-t 1 -c %f -g %f -r %f -d %f -q', C, gamma, coef0, degree);
    elseif ktype=="rbf" % _________ exp(-gamma*|u-v|^2)
        gamma = kp1; 
        lib_opt =  sprintf('-t 2 -c %f -g %f -q', C, gamma);
    elseif ktype=="sig" % _________ tanh(gamma*u'*v + coef0)
        gamma = kp1; 
        coef0 = 0; 
        lib_opt =  sprintf('-t 2 -c %f -g %f -r %f -h 0 -q', C, gamma, coef0);
...
%% Training 
	tt = tic;
    model = LIB_train_L2( Y , X , lib_opt );
    tr_time = toc(tt);
    
%% Predicting
    [mv,~] = size(ValX);       emv = ones(mv,1);
	[PredictY,~,decision_values] = LIB_predict_L2( emv , ValX , model ); 

%% Output 
%      model.w = model.SVs' * model.sv_coef;
%      model.b = -model.rho;
%     decision_values = TestX*model.w + model.b;

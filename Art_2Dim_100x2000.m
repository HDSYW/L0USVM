...
%% Import Data
Para.datName = 'Art_2Dim_100x2000';
mat_path = strcat('./00-DemoData/',Para.datName,'.mat');
Data = load(mat_path);
[m,n] = size(Data.X); Para.n = n;
...
for mod = 1:length(ModS)
    ...
    if sum(ModelName == "L0USVM")
        Para.rho = 6;                    Para.Update_rho = "OFF";
        Para.utype = 'MidPoint';         Para.inti_w = "OFF";
        Para.eta = 4;                    Para.L0USVM.UpdateRho = "OF";
        Para.itmax = 100;                Para.L0USVM.UpdateU = "ON";
        Para.WrokSet = "ON";             Para.L0USVM.WorkSetRatio = 80/100;
        Para.ITER = 1;

        indices = crossvalind('Kfold',Data.X(:,1),k);
        M1 = 2.^6; M2 = 2.^5; M3 = 2.^2; M4 = 10.^0;
        Para.p1=M1;Para.p2=M2;Para.p2=M2;Para.p2=M2;
    ...
    elseif sum(ModelName=="L1n_L1SVC")
        ...
        fprintf('───────────── %s Performance ─────────────\n', tt);
        fprintf('tAcc:%.4f  |  tGM:%.4f  |  tSen:%.4f  |  tSpe:%.4f\n', tAc, tGM, tSen, tSpe);
        fprintf('tTime:%.6f  |  tN_SV:%d\n', tTime, tN_SV);
        Para.tAc = tAc;
        fprintf('|  tspsN:%d(in%d)  |  tspsR:%.2f\n', ModT.spsN, n, ModT.spsR);
        fprintf('───────────────────────────────────\n');
    ...
    elseif sum(ModelName == "L2L1n_L1SVC")
        ...
        fprintf('%s Data Experment with Opt-Paras is Starting... \n', tt);
        fprintf('───────────── %s Performance ─────────────\n', tt);
        fprintf('tAcc:%.4f  |  tGM:%.4f  |  tSen:%.4f  |  tSpe:%.4f', tAc, tGM, tSen, tSpe);
        fprintf('tTime:%.6f  |  tN_SV:%d\n', tTime, tN_SV);
        Para.tAc = tAc;
        fprintf('|  tspsN:%d(in%d)  |  tspsR:%.2f\n', ModT.spsN, n, ModT.spsR);
        fprintf('───────────────────────────────────\n');
    ...
    elseif sum(ModelName == "USVM")
        ...
        M2 = 2.^8;     %Cu
        M3 = 0.001;       %epsilon
        Para.p1 = M1;Para.p2 = M2;Para.p3 = M3;
        [PredY, ModT] = ModelNameFun(Data.TstX , Data , Para);
        Para.data_TstY = Data.TstY;
        CM = ConfusionMatrix(PredY, Data.TstY);    tt = 'Test';
        [tAc, tGM, tSen, tSpe] = deal(CM.Ac, CM.GM, CM.Sen, CM.Spe);

...
clc;clear
%% Import Data
Para.datName = 'CCFD_10wx30';
mat_path = strcat('./00-DemoData/',Para.datName,'.mat');
Data = load(mat_path); [~,n] = size(Data.X); Para.n = n;
Para.run_name = mfilename;
Para.plt = 0;
% [Data.X,Data.Y,Data.TstX,Data.TstY] = TT(Data.X,Data.Y,0.7);
% [Data.TstX,Data.TstY,~,~] = TT(Data.TstX,Data.TstY,0.7);
% [~,~,Data.Ux,Data.Uy] = TT(Data.Ux,Data.Uy,0.1);
%% Paras
ModS = [];
% ModS = [ModS;"LIB_L1SVC"];
...
for mod = 1:length(ModS)
    ...
    if sum(ModelName == "L0USVM")
        ...
        Para.WrokSet = "ON";             Para.L0USVM.WorkSetRatio = 1/10;
        Para.ITER = 1;
